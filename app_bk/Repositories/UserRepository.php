...
class UserRepository extends BaseRepository
{
    ...
    public function getConversation($id, $input)
    {
        ...
        $unreadCount = $countQuery->where('status', '=', 0)->where('to_id', '=', (string)getLoggedInUserId())->count();
    ...
    public function myContactIds()
    {
        ...
        $records = ChatRequestModel::whereOwnerType(User::class)
            ...
            ->get(DB::raw("CASE WHEN from_id = $authId THEN owner_id ELSE from_id END AS my_contact_id, status"))
            ->groupBy('status');
        ...
        $conversationUserIds = Conversation::whereToType(Conversation::class)
            ...
            ->toBase()
            ->get(DB::raw("CASE WHEN from_id = $authId THEN to_id ELSE from_id END AS my_contact_id"))
            ->pluck('my_contact_id')
            ->toArray();
